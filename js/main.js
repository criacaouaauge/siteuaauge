...
jQuery.fn.parallax = function () {
  ...
  this.each(function () {
    ...
    if (winWidth > 1024) {
      ...
      parallaxContainer.css({
        'left': '0px',
        '-webkit-transform': 'translateY(' + Math.round((img.height() - bgobj.outerHeight()) / -2) + 'px) translateZ(0)',
        '-moz-transform': 'translateY(' + Math.round((img.height() - bgobj.outerHeight()) / -2) + 'px) translateZ(0)',
        '-ms-transform': 'translateY(' + Math.round((img.height() - bgobj.outerHeight()) / -2) + 'px) translateZ(0)',
        'transform': 'translateY(' + Math.round((img.height() - bgobj.outerHeight()) / -2) + 'px) translateZ(0)'
      });

      if (img.height() < parallaxContainer.height()) {
        img.css('width', img.width() * (bgobj.outerHeight() / img.height()));
        parallaxContainer.css({
          'left': +Math.round((img.width() - parallaxContainer.width()) / -2) + 'px',
...
function move_gradient() {
  $('.picture').each(function () {
    $(this).mousemove(function (e) {
      ...
      $('.grad').css({
        'transform': 'matrix(1, 0, 0, 1,' + Xinner + ',' + Yinner
      })
    })
  });
